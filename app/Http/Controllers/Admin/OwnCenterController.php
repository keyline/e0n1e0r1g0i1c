...
use Helper;
use Hash;
class OwnCenterController extends Controller
{
    public function __construct()
    {        
        $this->data = array(
            'title'             => 'Center',
            'controller'        => 'OwnCenterController',
            'controller_route'  => 'own-center',
            'primary_key'       => 'id',
        );
    }
    /* list */
        public function list(){
            ...
            $data['rows']                   = Center::where('status', '!=', 3)->orderBy('id', 'DESC')->get();
    ...
    /* add */
        public function add(Request $request){
            ...
            if($request->isMethod('post')){
                ...
                if($this->validate($request, $rules)){
                    ...
                    if($checkValue <= 0){
                        ...
                        /* center no generate */
                        if($postData['center_type_id'] == 1){
                            $center_owner_id = $postData['center_owner_id1'];
                        } else {
                            $center_owner_id = $postData['center_owner_id2'];
                        }
                        $fields = [
                            'sl_no'                     => $next_sl_no,
                            'center_no'                 => $center_no,
                            ...
                            'photo'                     => $photo,
                            'created_by'                => 'ADMIN',
                            'center_type_id'            => $postData['center_type_id'],
                            'center_owner_id'           => $center_owner_id,
                        ];
            ...
            $data['centerTypes']            = CenterType::select('id', 'name')->where('status', '=', 1)->get();
            $data['centerOwners']           = FranchiseOwner::select('id', 'name')->where('status', '=', 1)->get();
