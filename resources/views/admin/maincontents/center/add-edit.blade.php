...
<section class="section profile">
  <div class="row">
    ...
    if($row){
      ...
      $aadhar_doc_id          = $row->aadhar_doc_id;
      $aadhar_doc             = $row->aadhar_doc;
      $qualification_doc_id   = $row->qualification_doc_id;
      $qualification_doc      = $row->qualification_doc;
      $photo                  = $row->photo;
      $center_type_id         = $row->center_type_id;
      $center_owner_id        = $row->center_owner_id;
    } else {
      $name                   = '';
      $address                = '';
      $country_id             = '';
      ...
      <div class="card">
        <div class="card-body pt-3">
          <form method="POST" action="" enctype="multipart/form-data">
            @csrf
            <div class="row mb-3">
              <label for="center_type_id" class="col-md-2 col-lg-2 col-form-label">Center Type</label>
              <div class="col-md-10 col-lg-10">
                  <select name="center_type_id" class="form-control" id="center_type_id" required>
                    <option value="" selected>Select Center Type</option>
                    <?php if($centerTypes){ foreach($centerTypes as $ct){?>
                    <option value="<?=$ct->id?>" <?=(($ct->id == $center_type_id)?'selected':'')?>><?=$ct->name?></option>
                    <?php } }?>
                  </select>
              </div>
            </div>
            <div class="row mb-3" id="own-owner" style="display: none;">
              <input type="hidden" name="center_owner_id1" value="0" id="center_owner_id1">
            </div>
            <div class="row mb-3" id="franchise-owner">
              <label for="center_owner_id2" class="col-md-2 col-lg-2 col-form-label">Center Owner</label>
              <div class="col-md-10 col-lg-10">
                  <select name="center_owner_id2" class="form-control" id="center_owner_id2">
                    <option value="" selected>Select Center Owner</option>
                    <?php if($centerOwners){ foreach($centerOwners as $ct){?>
                    <option value="<?=$ct->id?>" <?=(($ct->id == $center_owner_id)?'selected':'')?>><?=$ct->name?></option>
                    <?php } }?>
                  </select>
              </div>
            </div>
            ...
            <div class="row mb-3">
              <label for="id_proof" class="col-md-2 col-lg-2 col-form-label">ID Proof (PAN)</label>
              <input type="hidden" name="doc_type_id" value="5">
              <div class="col-md-10 col-lg-10">
                <input type="file" name="id_proof" class="form-control" id="id_proof">
                <small class="text-info">* Only PDF & image files are allowed</small><br>
                <?php if($id_proof != ''){?>
            ...
            <div class="row mb-3">
              <label for="aadhar_doc" class="col-md-2 col-lg-2 col-form-label">ID Proof (Aadhar)</label>
              <input type="hidden" name="aadhar_doc_id" value="1">
              <div class="col-md-10 col-lg-10">
                <input type="file" name="aadhar_doc" class="form-control" id="aadhar_doc">
                <small class="text-info">* Only PDF & image files are allowed</small><br>
                <?php if($aadhar_doc != ''){?>
                  <a href="<?=env('UPLOADS_URL').'center/'.$aadhar_doc?>" target="_blank" class="badge bg-primary" download>View File</a>
                <?php }?>
              </div>
            </div>
            <div class="row mb-3">
              <label for="qualification_doc" class="col-md-2 col-lg-2 col-form-label">ID Proof (Qualification Docs)</label>
              <input type="hidden" name="qualification_doc_id" value="6">
              <div class="col-md-10 col-lg-10">
                <input type="file" name="qualification_doc" class="form-control" id="qualification_doc">
                <small class="text-info">* Only PDF & image files are allowed</small><br>
                <?php if($qualification_doc != ''){?>
                  <a href="<?=env('UPLOADS_URL').'center/'.$qualification_doc?>" target="_blank" class="badge bg-primary" download>View File</a>
                <?php }?>
              </div>
            </div>
...
<script type="text/javascript">
  $(document).ready(function(){
    ...
    var center_type_id = '<?=$center_type_id?>';
    if(center_type_id == 1){
      $('#own-owner').show();
      $('#franchise-owner').hide();
      $('#center_owner_id1').attr('required', true);
      $('#center_owner_id2').attr('required', false);
    } else {
      $('#own-owner').hide();
      $('#franchise-owner').show();
      $('#center_owner_id1').attr('required', false);
      $('#center_owner_id2').attr('required', true);
    }
    $('#center_type_id').on('change', function(){
      var center_type_id = $('#center_type_id').val();
      if(center_type_id == 1){
        $('#own-owner').show();
        $('#franchise-owner').hide();
        $('#center_owner_id1').attr('required', true);
